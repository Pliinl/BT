...
function searchVehicle() {
  ...
  for (let i in vehicleArray) {
    ...
    if (vehicle.vehicleID == searchID) {
      vehicle.showInfo();
      alert(`Owner Name - Tên chủ sở hữu: ${vehicle.ownerName}.\n Vehicle Type - Loại phương tiện: ${vehicle.vehicleType}.\nCylinder Capacity: Dung tích xy lanh: ${vehicle.cylinderCapacity}.\n Tax Vlaue - Giá trị thuế: ${vehicle.taxValue()}`);
      break;
    }
  }
}
...
function updataVehicle() {
  ...
  for (let i in vehicleArray) {
    ...
    if (vehicle.vehicleID == vehicleID) {
      ...
      vehicle.cost = cost;
      alert("Vehicle is updated successfully!- Xe đã được cập nhật thành công!")
      updateVehicleTable(vehicleArray);
      break;
    }
  }
